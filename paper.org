...
\bfseries 
  ...
  the /awake/ state) determining those properties for neocortical
  ...
  sharpness of sodium activation and spike frequency inactivation
  might be the main contributors to the observed diversity of firing
  ...
  inhomogeneous in young mice visual cortex, what would critically
  impact cortical computation at early stages of sensory processing.
...
of the characteristics of the fluctuations. By highlighting and
quantifying the diversity of cellular computational properties, our
study contributes to the understanding of how biological heterogeneity
impacts neocortical computation of sensory input.
...
form the basis of cortical computation [[cite:Destexhe2006]]. Since
sensory processing of natural stimuli also evoke sparse response at
low population rates, see for example [[citetext:Baudot2013]] in cat V1,
...
a difficulty for experimentalists /in vivo/ as, by definition,
...
of single neurons which was made possible by the combination of the
...
[[cite:LaCamera2008]], our approach allows to investigate the response to
fast membrane potential fluctuations present in the /high conductance/
state of cortical networks [[cite:Destexhe2003]]. This characterization
focuses on how fluctuations are translated into output spikes on top
of subthreshold integration effects [[cite:Kuhn2004]]. In addition, we
...
[[fig:3D-motiv]]B). Nevertheless, the time constant taken from an
...
function (gray area in the inset of Figure [[fig:3D-motiv]]B):
...
[[cite:Destexhe2001a,Fernandez2011]]. We chose the following stimulation.
...
the theoretical models proposed in [[citetext:Brette2005a]] and
[[citetext:Platkiewicz2011]].
...
[[cite:McCormick1985]] and $\theta(t)$ is a variable threshold whose
...
[[cite:Amit1997]]. Our situation is different, we have a more complex
...
[[cite:Zerlaut2015]]. It contains the data from all cells, the fitting
...
#+CAPTION: *Investigating somatic computation in the fluctuation-driven regime* *(A)* Schematic illustration on a layer V pyramidal cell in cat V1 [[cite:Contreras1997]] together with theoretical distribution of inhbitiory (red) and excitatory (green) synaptic inputs. Synaptic and dendritic integration of pre-synaptic spike trains (two sample spike trains in upper right) produce membrane potential fluctuations at the soma as recorded intracellularly in current-clamp. *(B)* A sample trace of membrane potential integrating inputs from all inhibitory and excitatory synapses (black trace). We characterize those fluctuations by a mean \mu_V (dashed horizontal line), a standard deviation \sigma_V of sub-threshold oscillations (gray background) and global autocorrelation time \tau_V (gray background, determined from the half integral of the normalized autocorrelation function in the inset, see Methods[[sec:autocorrel-def]]). The properties of those fluctuations determine the spiking probability of the neuron (three spikes visibile in the membrane-potential trace).
#+NAME: fig:3D-motiv
...
investigate the firing response as a function of those variables
(illustrated on Figure [[fig:3D-motiv]]).
...
[[cite:Kuhn2004]].  This procedure allows to focus on how active currents
...
One approach consists in fitting the response to the formula derived from
 ...
 [[cite:Rauch2003,Lundstrom2009a]]. This strategy has three drawbacks: 1)
 ...
 (e.g. reproducing synaptic dynamics, see [[citetext:Brunel1998a]]) and 3)
 ...
 membrane capacitance) are free parameters to have enough degree of
 freedom.
 ...
 [[citetext:Amit1997]], for the firing rate response of the LIF model:
...
\end{equation}
 ...
 [[cite:Amit1997]], we take here the global autocorrelation time instead
...
the model of [[citetext:Brette2005a]] by adding an inactivation mechanism
[[cite:Platkiewicz2011]]. Several widespread theoretical models are
special cases of this model: the leaky integrate and fire (LIF), the
exponential integrate and fire (EIF, [[citetext:Fourcaud-Trocme2003]]), the
inactivating leaky integrate and fire (iLIF, [[citetext:Platkiewicz2011]]). We
...
parameter), linear function (4 parameters) and second-order polynomial
...
effects are captured by the phenomenological threshold. For example,
...
deviation \sigma_V and speed of the fluctuations \tau_V^N. The
...
[[fig:illustration-sensitivities]]).
 ...
 The response of the LIF model provides a basic picture for the firing
 rate response (see LIF in Figure [[fig:model-response-and-fit]] and
 Figure [[fig:illustration-sensitivities]]A). Spiking increases with mean
 ...
 (exponential activation, adaptation, etc.) affect those
 characteristics.  First of all, those mechanisms suppress spiking and
 ...
 to extract fast varying fluctuations [[cite:Fourcaud-Trocme2003]], is
 ...
 [[cite:McCormick1985]]. This is an effect that attenuates firing and
 because it is proportional to firing itself it should reduce the
 dependencies to all variables. Indeed the sensitivities to \mu_V and
 \sigma_V are strongly attenuated with respect to LIF (Figure
 [[fig:illustration-sensitivities]]C-D). In contrast, the sensitivity 
 \tau_V^N is only mildly affected. The temporal dynamics of the hyperpolarizing current
 (\tau_W=500ms) penalizes temporally neighboring output spikes, consequently slow fluctuations are more strongly
 dampened than fast fluctuations which restores the sensitivity to
 \tau_V^N (Figure [[fig:illustration-sensitivities]]E).

 The iLIF model reproduce the fast inactivation properties of sodium
 channels (see [[citetext:Hille2001]] for a review). Close to threshold,
...
recordings [[cite:Rauch2003,Kondgen2008a]].  Both to obtain this stability
...
/perforated patch/ technique [[cite:Rae1991,Lippiat2008]], in which
...
of membrane [[cite:Wendt1992,Kyrozis1995]].  Although the technique may
...
membrane resistance (4.7 \pm 2.6 %, see also [[citetext:Rae1991]]), thus
...
first slice of data would give the same phenomenological threshold as
the second slice of data. We found a good agreement if the number of
scanned configuration of input space (\mu_V, \sigma_V, \tau_V) was
n_{points} $\geq$ 70 (Pearson correlations, c>0.8 and p<1e-5 for the
correlations between first and second slice of data, Figure
[[fig:data-response-and-fit]]B). The high Pearson correlations between the
response characteristics in the first and second slice of data
indicates that the characterization is robust for a slice of data
having n_{points} $\geq$ 35. Among the 30 cells used for further
...
[[fig:data-response-and-fit]]A. Cells 1 and cell 4 show a very
...
for cell 2 than for cells 3 and 4. Also the sensitivity to \mu_V
seems to be variable, a 10 mV depolarization has stronger effect
on responses of cell 4 than cell 3. Finally, the
...
dimensional projections, the data semt to show some degree of
covariations (Pearson correlation analysis in Figure
[[fig:cellular-heterogeneity]]B). We then looked for the four-dimensional
structure of those covariations in the response characteristics by
...
heterogeneity, suggesting that the structure is relatively weak.

Nonetheless, two vectors explained 75% of the variations in the
data. The first vector corresponds to a covariation of the
sensitivities to mean and amplitude of the fluctuations. This
comodulation can be achieved in the sfaLIF model, varying the weight
...
second vector corresponds to a covariation of a decrease in the
...
Compared to previous studies [[cite:LaCamera2008]], we focused on the low
...
[[cite:LaCamera2008,Giugliano2008]]. Our approach can be seen as a way to
...
that 1) it allows to compare individual cells and 2) as a function of
the current and the conductance, the spiking response is dominated by
subthreshold integration effects, e.g. for a same variance of input
current, an increasing conductance would decrease the V_m fluctuations
amplitude by shunting effects [[cite:Kuhn2004]]. The effects of the
spike-related mechanisms are then difficult to de-intricate from those
subthreshold effects. Here, we used analytical calculus to design a
stimulation protocol that allows to control the fluctuations and
therefore to focus on understanding the single neuron computation on
top of the fluctuations properties.
...
[[cite:Brunel1998a,Platkiewicz2010]]. Unlike the two mentioned studies,
...
Also, many cells showed a weaker sensitivity to the speed of the
...
pyramidal neurons in rat neocortex [[cite:Kondgen2008a,Ilin2013]], where
...
dynamic I-V curve analysis [[cite:Badel2008]] rendering this possibility
unlikely.
...
analogous to the phenomena described in [[citetext:Fernandez2011]] for
...
properties emerging at the network level, which constitute the main perspective for future
